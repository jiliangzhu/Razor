...
1) **WS `book` 解析只支持 string，若字段为 number 会把盘口退化成 bid=0 / ask=1** ✅ 已修复
...
2) **WS `book` 使用消息里的 `market` 字段作为 market_id，可能与 token→market 映射不一致** ✅ 已修复
...
3) **market_select 的 trades 时间戳域与 Phase1 冻结口径不一致（probe 里用 exchange_ts 优先）** ✅ 已修复
...
4) **day14_report 当前按“notes 组合键”分组，而不是按“单个 reason code”拆分统计** ✅ 已修复
...
1) **多个 `tokio::time::interval` 未设置 `MissedTickBehavior::Delay`，卡顿后可能 burst “追账式”触发** ✅ 已修复
...
2) **Brain 的去重表 `last_by_key` 可能无界增长（长期运行累积很多 (market,strategy,cost_bucket) key）** ✅ 已修复
...
3) **TradeStore `trim()` 隐含“push 基本按时间单调递增”的假设** ✅ 已修复
...
4) **bucket 的 depth3 degraded 判据未覆盖“单位误读导致 depth3 巨大”的情况** ✅ 已修复
...
1) **market_select 目前是“全部 probe 完成后才落盘”，若进程异常退出会丢已完成 probe 结果** ✅ 已修复（Ctrl‑C/部分输出）
...
2) **缺少 config 自检（防止手滑导致长期静默退化）** ✅ 已修复（`Config::validate()`）
...
3) **trades poller 固定 `takerOnly=true` 可能系统性低估成交量** ✅ 已修复（已配置化）
